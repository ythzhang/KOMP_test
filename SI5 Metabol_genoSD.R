...
# Starting TwoStage significance assessment using Two-way ANOVA
  ...
  Normality_before = Normality_after = Normality_F = Normality_M = c()
  numb_null = numb_gene = c()
  pvalue_Stage1 = pvalue_Stage2 = pvalue_Stage3 = c()
  sep_allKO_pval = sep_allKO_estimate = c() 
  sep_FvKO_pval = sep_FvKO_estimate = sep_MvKO_pval = sep_MvKO_estimate = c()
  fdr_Stage1 = fdr_Stage2 = fdr_Stage3 = c()
  fdr_ctrl_sex = fdr_KO_sex = c() 
  foldchange_all = foldchange_FvKO = foldchange_MvKO = c()
  ctrl_sex_pval = ctrl_sex_estimate = c() 
  KO_sex_pval = KO_sex_estimate = c()
  ...
  for(i in 1 : nrow(All_Metabolomics_e_no_mising)){
      ...
      data_raw <- data.table(y = All_Metabolomics_e_no_mising[i, ], sex = All_Metabolomics_p$Gender, genotype = All_Metabolomics_p$Genotype)
      colnames(data_raw) <- c("Intensity_raw", "sex", "genotype")
      ...
      Normality_before[i] <- shapiro.test(data_raw$Intensity_raw)$p.value
      # tranform data
      ...
      if(length(model_afterFIXED) <= 1){
        ...
        pvalue_Stage1b[i] <- NA
        pvalue_Stage2[i] <- NA
        ctrl_sex_pval[i] = ctrl_sex_estimate[i] <- NA 
        KO_sex_pval[i] = KO_sex_estimate[i] <- NA
        sep_allKO_pval[i] = sep_allKO_estimate[i] <- NA
        sep_FvKO_pval[i] = sep_FvKO_estimate[i] <- NA
        sep_MvKO_pval[i] = sep_MvKO_estimate[i] <- NA
        
        fdr_Stage1 = fdr_Stage2 = fdr_Stage3 <- NA
        ...
        Normality_after[i] <- shapiro.test(dataset$Intensity)$p.value
        Normality_F[i] <- shapiro.test(dataset$Intensity[dataset$sex %in% "Female"])$p.value
        ...
        #p-values
        pvalue_Stage1[i] <- geno_effect
        pvalue_Stage2[i] <- sex_effect
        pvalue_Stage3[i] <- Interct_effect
        ...
        if(length(result_all) <= 1){
          sep_allKO_pval[i] <- NA
          sep_allKO_estimate[i] <- NA
        }else{
          sep_all <- nlme:::summary.gls(result_all)$tTable 
          sep_allKO_pval[i] <- sep_all[2, 4]
          sep_allKO_estimate[i] <- sep_all[2, 1]
        }
        ...
        if(length(result_F) <= 1){
          sep_FvKO_pval[i] <- NA
          sep_FvKO_estimate[i] <- NA
        }else{
          sep_FvKO <- nlme:::summary.gls(result_F)$tTable 
          sep_FvKO_pval[i] <- sep_FvKO[2, 4]
          sep_FvKO_estimate[i] <- sep_FvKO[2, 1]
        }
        ...
        if(length(result_M) <= 1){
          sep_MvKO_pval[i] <- NA
          sep_MvKO_estimate[i] <- NA
        }else{
          sep_MvKO <- nlme:::summary.gls(result_M)$tTable 
          sep_MvKO_pval[i] <- sep_MvKO[2, 4]
          sep_MvKO_estimate[i] <- sep_MvKO[2, 1]
        }
        ...
        if(length(ctrl_test) <= 1){
          KO_sex_pval[i] <- NA
          KO_sex_estimate[i] <- NA
        }else{
          ctrl_result <- nlme:::summary.gls(ctrl_test)$tTable 
          ctrl_sex_pval[i] <- ctrl_result[2, 4]
          ctrl_sex_estimate[i] <- ctrl_result[2, 1]
        }
        ...
        if(length(KO_test) <= 1){
          ...
          KO_sex_pval[i] <- KO_result[2, 4]
          KO_sex_estimate[i] <- KO_result[2, 1]
        }
      }
      
fdr_Stage1[i] = p.adjust(pvalue_Stage1[i],'fdr')
fdr_Stage2[i] = p.adjust(pvalue_Stage2[i],'fdr')
fdr_Stage3[i] = p.adjust(pvalue_Stage3[i],'fdr')

fdr_ctrl_sex[i] = p.adjust(ctrl_sex_pval[i],'fdr')
fdr_KO_sex[i] = p.adjust(KO_sex_pval[i],'fdr')
    }

# calculate significance ratio
sum(pvalue_Stage1<0.05, na.rm = TRUE)  
