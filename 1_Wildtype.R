...
for (i in 1 : nrow(Phenotype_UCDavis_e_null)){
  ...
  if(continuous){
    ...
    p_val3[i] <- summary(lm(Phenotype_UCDavis_e_Null_Stats ~ Phenotype_UCDavis_p_Null_Stats$Gender, na.action = 'na.exclude'))$coefficients[2, 4]
    Coef3[i] <- summary(lm(Phenotype_UCDavis_e_Null_Stats ~ Phenotype_UCDavis_p_Null_Stats$Gender, na.action = 'na.exclude'))$coefficients[2, 1]
