...
for (i in 1:nrow(Null_Metabolomics_e)){
  ...
  Null_Metabol$Blom<- rankNorm(Null_Metabol$Intensity)
  # plotNormalHistogram(Null_Metabolomics_e_Blom)
  test2_Norma[i] <- shapiro.test(Null_Metabol$Blom)$p.value
  ...
  outliers_nullF<-boxplot.stats(Null_Metabol$Blom[Null_Metabol$Gender%in%'Female'])$out
  outliers_nullM<-boxplot.stats(Null_Metabol$Blom[Null_Metabol$Gender%in%'Male'])$out
  
  if( length(outliers_nullF)>0 & length(outliers_nullM)>0){
    
  Label_outlier<-c(which(Null_Metabol$Blom %in% outliers_nullF[[1]]),which(Null_Metabol$Blom %in% outliers_nullM[[1]]))
  Null_Metabol_new <- Null_Metabol [-Label_outlier,]
  
  }else if(length(outliers_nullF)>0 & length(outliers_nullM)==0){
    
    Label_outlier<-c(which(Null_Metabol$Blom %in% outliers_nullF[[1]]))
    Null_Metabol_new<-Null_Metabol [-Label_outlier, ]

    
  }else if(length(outliers_nullF)==0 & length(outliers_nullM)>0){
    
    Label_outlier<-c(which(Null_Metabol$Blom %in% outliers_nullM[[1]]))
    Null_Metabol_new <- Null_Metabol[-Label_outlier, ]
  ...
  Normality_M[i]<-shapiro.test(Null_Metabol_new_M$Blom)$p.value
  Null_Metabol_new_F <-  Null_Metabol_new[Null_Metabol_new$Gender %in% "Female"]
  if( length(unique(Null_Metabol_new_F))==1 ){
    Normality_F[i] <- NA
    
  }else{
    Normality_F[i]<-shapiro.test(Null_Metabol_new_F$Blom)$p.value 
  }
...
Phenotype_UCDavisSelect = wcmc::read_data("C:\\Users\\yzhan\\Documents\\Fiehn\\3_KOMP\\7_Sexual D_20190605\\Jennly Zhang KOMP SD_ver09\\Input\\Phenotype_UCDavisSelect.xlsx")
...
for (i in 1:nrow(Null_Metabolomics_e)){
  ...
  Null_Metabolomics_e_Blom<- rankNorm(Null_Metabolomics_e[i,])
  # plotNormalHistogram(Null_Metabolomics_e_Blom)
  test2_Norma[i] <- shapiro.test(Null_Metabolomics_e_Blom)$p.value
  
  outliers_nullF<-boxplot.stats(Null_Metabolomics_e_Blom[Null_Metabolomics_p$Gender%in%'Female'])$out
  outliers_nullM<-boxplot.stats(Null_Metabolomics_e_Blom[Null_Metabolomics_p$Gender%in%'Male'])$out
  
  if( length(outliers_nullF)>0 & length(outliers_nullM)>0){
    
    Label_outlier<-c(which(Null_Metabolomics_e_Blom%in%outliers_nullF[[1]]),which(Null_Metabolomics_e_Blom%in%outliers_nullM[[1]]))
    Null_Metabolomics_e_Blom_New<-Null_Metabolomics_e_Blom[-Label_outlier]
    Null_raw_e_new <-Null_Metabolomics_e[i,][-Label_outlier]
    Null_Metabolomics_p_New<-Null_Metabolomics_p[-Label_outlier]
    
  }else if(length(outliers_nullF)>0 & length(outliers_nullM)==0){
    
    Label_outlier<-c(which(Null_Metabolomics_e_Blom%in%outliers_nullF[[1]]))
    Null_Metabolomics_e_Blom_New<-Null_Metabolomics_e_Blom[-Label_outlier]
    Null_raw_e_new <-Null_Metabolomics_e[i,][-Label_outlier]
    Null_Metabolomics_p_New<-Null_Metabolomics_p[-Label_outlier]
    ...
    Label_outlier<-c(which(Null_Metabolomics_e_Blom%in%outliers_nullM[[1]]))
    Null_Metabolomics_e_Blom_New<-Null_Metabolomics_e_Blom[-Label_outlier]
    Null_raw_e_new <-Null_Metabolomics_e[i,][-Label_outlier]
    Null_Metabolomics_p_New<-Null_Metabolomics_p[-Label_outlier]
    
  }else{
    Null_Metabolomics_e_Blom_New<-Null_Metabolomics_e_Blom
  ...
  Null_Metabolomics_e_Stats<-data.table(Null_Metabolomics_p_New$Gender,Null_raw_e_new,Null_Metabolomics_e_Blom_New,body_weight[names(body_weight)%in%Null_Metabolomics_p_New$label])
...
setwd('C:\\Users\\yzhan\\Documents\\Fiehn\\3_KOMP\\7_Sexual D_20190605\\Jennly Zhang KOMP SD_ver09\\Output')
fwrite(data.table(label = Null_Metabolomics_f$label, Metbaolites = Null_Metabolomics_f$CompoundName,Platform= Null_Metabolomics_f$Assay, InChiKey = Null_Metabolomics_f$InChiKey, PubChemID = Null_Metabolomics_f$PubChemID, SMILES=Null_Metabolomics_f$SMILES, F_Count=F_Count, M_Count =M_Count, Normality_before = test1_Norma, Normality_before = test2_Norma, Normality_M = Normality_M, Normality_F = Normality_F,p_value = p_val2, Coefficient=Coef2,fold_change = fc2, adjusted_p_value = p_val_adj2),"1b Metabol_adjust byBW(norm)_outlier_Normality.csv")
